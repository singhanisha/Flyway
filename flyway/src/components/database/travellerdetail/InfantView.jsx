import { useEffect, useState } from "react";

const InfantView = ()=>{

    const [infant,setInfant] = useState([]);
    const [travel,setTravel] = useState([]);
    const [ message,setMessage] = useState('');
    const [ show,setShow] = useState(true)

    const INFANTSECTION_API_URL = "http://localhost:8080/api/infantsections";
    const TRAVEL_API_URL = "http://localhost:8080/api/travel";

    const getAllInfo = (serverUrl)=>{

        fetch( serverUrl,{
            method:"GET",
            headers:{ 
                'Accept':'application/json'
            }
        }) 
            .then( response=>{
                if(response.ok)
                    return response.json()
                else if(response.status=="404")
                    return response.json()
                else
                    throw Error(`Server Error ${response.status}`)
            }) 
            .then( data=> setTravel(data)) 
            .catch( err=> console.error(err)) 
    }
    useEffect( ()=>{

        getAllInfo(TRAVEL_API_URL+"/all-travel");

    },[travel]);

    //infant
    const getAllInfo3 = (serverUrl)=>{
        fetch( serverUrl,{
            method:"GET",
            headers:{ 
                'Accept':'application/json'
            }
        }) 
            .then( response=>{
                if(response.ok)
                    return response.json()
                else if(response.status=="404")
                    return response.json()
                else
                    throw Error(`Server Error ${response.status}`)
            }) 
            .then( data=> setInfant(data)) 
            .catch( err=> console.error(err)) 
    }
    useEffect( ()=>{

        getAllInfo3(INFANTSECTION_API_URL+"/all-infantsection");

    },[infant]);

    return(
        <>
        <table>
            <thead>
                <tr style={{textAlign:'center'}}>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Infant Name</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Infant Gender</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Date_of_Birth</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Email</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Mobile Number</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Address</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>State</th>
                    <th style={{backgroundColor:'rgb(194, 72, 16)',color:'white'}}>Pincode</th>
                </tr>
            </thead>
            <tbody>
                {
                    infant.map(i=> <tr style={{textAlign:'center'}}>
                        <td style={{backgroundColor:'wheat'}}>{i.infantname} {i.infantlastName}</td>
                        <td style={{backgroundColor:'wheat'}}>{i.infantgender}</td>
                        <td style={{backgroundColor:'wheat'}}>{i.idateofbirth}</td>
                        <td style={{backgroundColor:'wheat'}}>{ travel.map(t=> <tr key={t.id}>
                                                    <td style={{border:'none'}}>{t.email}</td></tr>)}</td>
                        <td style={{backgroundColor:'wheat'}}>{ travel.map(t=> <tr key={t.id}>
                                                    <td style={{border:'none'}}>{t.mbNo}</td></tr>)}</td>
                        <td style={{backgroundColor:'wheat'}}>{ travel.map(t=> <tr key={t.id}>
                                                    <td style={{border:'none'}}>{t.address}</td></tr>)}</td>
                        <td style={{backgroundColor:'wheat'}}>{ travel.map(t=> <tr key={t.id}>
                                                    <td style={{border:'none'}}>{t.state}</td></tr>)}</td>
                        <td style={{backgroundColor:'wheat'}}>{ travel.map(t=> <tr key={t.id}>
                                                    <td style={{border:'none'}}>{t.pincode}</td></tr>)}</td>
                    </tr>)
                }
            </tbody>
        </table>
        </>
    )
}
export default InfantView;